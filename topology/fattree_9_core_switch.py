...
class DockerHost( Host ):
    "Docker host"
    def __init__( self, name, image='pankajm/hadoopimg:1.4', dargs=None, startString=None, **kwargs ):
        self.image = image
        self.dargs = dargs
...
class HugeTopo(Topo):
    ...
    def __init__(self):
        ...
        self.iHost = self.iEdgeLayerSwitch * 3 
    ...
    def createLink(self):
        ...
        for x in range(0, self.iAggLayerSwitch, 3):
        ...
        for x in range(1, self.iAggLayerSwitch, 3):
            self.addLink(self.CoreSwitchList[3], self.AggSwitchList[x], bw=1000, )#loss=5)
            self.addLink(self.CoreSwitchList[4], self.AggSwitchList[x], bw=1000, )#loss=5)
            self.addLink(self.CoreSwitchList[5], self.AggSwitchList[x], bw=1000, )#loss=5)

        for x in range(2, self.iAggLayerSwitch, 3):
            self.addLink(self.CoreSwitchList[6], self.AggSwitchList[x], bw=1000, )#loss=5)
            self.addLink(self.CoreSwitchList[7], self.AggSwitchList[x], bw=1000, )#loss=5)
            self.addLink(self.CoreSwitchList[8], self.AggSwitchList[x], bw=1000, )#loss=5)
        ...
        for x in range(0, self.iAggLayerSwitch, 3):
            ...
            self.addLink(self.AggSwitchList[x], self.EdgeSwitchList[x+2], bw=1000,)
            ...
            self.addLink(self.AggSwitchList[x+1], self.EdgeSwitchList[x+2], bw=1000,)
            self.addLink(self.AggSwitchList[x+2], self.EdgeSwitchList[x+2], bw=1000,)
            self.addLink(self.AggSwitchList[x+2], self.EdgeSwitchList[x+1], bw=1000,)
            self.addLink(self.AggSwitchList[x+2], self.EdgeSwitchList[x], bw=1000,)

        logger.debug("Create Edge to Host")
        for x in range(0, self.iEdgeLayerSwitch):
            ## limit = 2 * x + 1 
            self.addLink(self.EdgeSwitchList[x], self.HostList[3 * x], bw=1000,)
            self.addLink(self.EdgeSwitchList[x], self.HostList[3 * x + 1], bw=1000,)
            self.addLink(self.EdgeSwitchList[x], self.HostList[3 * x + 2], bw=1000,)
